...
if ismember(3,variables)
    ...
    total_ind = total_ind + param_ind;
    param1 = param(total_ind - param_ind + 1:total_ind);
    [speed_y,speed_x] = spline_1d_plot(param1,ctl_pts_all{3},s);
    scale(3) = mean(exp(speed_y));
end
...
if plotfig
    ...
    if ismember(2,variables)
        ...
        plot(hd_x,exp(hd_y)*exp(b0)*prod(scale_factor)/dt,'k','linewidth',2);
        box off
        xlabel('HD angle')
        ylabel('spikes/s')
        axis tight
        tuning_curves{2} = exp(hd_y')*exp(b0)*prod(scale_factor);
        title('LNP hd tuning')
    end
